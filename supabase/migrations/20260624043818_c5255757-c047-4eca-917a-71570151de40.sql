...
-- ============================================================================
-- HANDOVER PACK INTEGRITY — view + validator (rerunnable, idempotent)
-- ============================================================================

-- Validated handover export view. Uses funding_type that already exists on
-- public.applications. Splits full_name into first/last deterministically.
...
-- Validator. Admin-gated. Returns jsonb { ok, totals, errors[] }.
...
BEGIN
  ...
  -- Missing names / surnames / student numbers / funding source / orphan profile
  FOR _row IN
    SELECT v.application_id, v.student_name, v.student_surname,
           v.student_number, v.funding_source, v.user_id, v.residence_id, v.residence_name
  ...
  LOOP
    ...
    IF _row.student_surname IS NULL THEN
      _errors := _errors || jsonb_build_object('code','missing_surname',
        'application_id', _row.application_id, 'reason','Student surname is blank (full_name has no space)');
    END IF;
    ...
    IF _row.funding_source IS NULL OR _row.funding_source = 'unknown' THEN
      _errors := _errors || jsonb_build_object('code','missing_funding',
        'application_id', _row.application_id, 'reason','Funding source is blank or unknown');
    END IF;
    IF _row.residence_name IS NULL THEN
      _errors := _errors || jsonb_build_object('code','invalid_residence',
        'application_id', _row.application_id, 'reason','Application references a residence that no longer exists');
    END IF;
  ...
  -- Duplicate student_number within the same residence
  FOR _row IN
  ...
  -- Duplicate applications (same user + same residence)
  FOR _row IN
  ...
  LOOP
    _errors := _errors || jsonb_build_object('code','duplicate_application',
      'application_id', _row.app_ids[1],
      'reason','User has multiple applications to the same residence: ' || array_to_string(_row.app_ids, ', '));
  END LOOP;
