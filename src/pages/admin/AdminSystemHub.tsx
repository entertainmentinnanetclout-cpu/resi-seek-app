...
import ResKonnectBrainControl from "@/components/admin/ResKonnectBrainControl";
...
const legacy:Record<string,string>={
  adminos:"automation",automation:"automation",intelligence:"automation",dimpho:"automation",luna:"automation",growth:"automation",
...
const AdminSystemHub=()=>{
  ...
  return <AdminLayout>
    <SEO title="Admin Command Hubs | ResKonnect" description="ResKonnect communications, AI intelligence, automation, operations and platform control hubs"/>
    <div className="adminos-command-centre min-w-0 max-w-full space-y-6 overflow-x-hidden">
      <section className="max-w-full overflow-hidden rounded-[30px] border bg-gradient-to-br from-background to-muted/35 p-5 shadow-sm sm:p-6"><div className="flex min-w-0 flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><div className="min-w-0"><div className="flex max-w-full flex-wrap gap-2"><Badge className="rounded-full">Simplified AdminOS</Badge><Badge variant="outline" className="rounded-full">4 command hubs</Badge><Badge variant="outline" className="rounded-full">Luna AgentOS RG0–RG2</Badge><Badge variant="outline" className="rounded-full">Dimpho Intelligence R1–R8</Badge></div><h1 className="mt-3 break-words text-3xl font-black tracking-tight">ResKonnect Command Centre</h1><p className="mt-2 max-w-3xl break-words text-sm leading-6 text-muted-foreground">One AdminOS now governs a shared ResKonnect Brain across customer-service channels. Luna and Dimpho keep their own identities, while verified knowledge, customer memory, intent history, governed tools and outcomes are shared across website, WhatsApp, email, voice and future Meta agents.</p></div><div className="grid min-w-0 grid-cols-2 gap-2 text-center sm:grid-cols-4"><MiniStat value="4" label="Hubs"/><MiniStat value="1" label="Shared Brain"/><MiniStat value="24/7" label="Automation"/><MiniStat value="Live" label="Supabase"/></div></div></section>
      <Tabs value={activeTab} onValueChange={setTopLevelTab} className="min-w-0 max-w-full overflow-x-hidden"><TabsList className="grid h-auto w-full min-w-0 grid-cols-2 gap-1 rounded-2xl bg-muted/60 p-1 lg:grid-cols-4">{tabs.map((t)=><TabsTrigger key={t.value} value={t.value} className="min-w-0 gap-2 rounded-xl py-2.5"><t.icon className="h-4 w-4 shrink-0"/><span className="truncate">{t.label}</span></TabsTrigger>)}</TabsList>
        ...
        <TabsContent value="automation" className="min-w-0 max-w-full space-y-5 overflow-x-hidden"><HubPanel title="ResKonnect Brain" description="Shared customer-service intelligence across Luna, Dimpho, email, voice, AdminOS and future Meta agents." icon={Brain} defaultOpen><ResKonnectBrainControl/></HubPanel><HubPanel title="Luna Growth Intelligence" description="RG0–RG2 control plane for Luna identity separation, campaign attribution, demand telemetry and scheduled demand ranking." icon={Sparkles} defaultOpen><AdminOSLunaGrowth/></HubPanel><HubPanel title="Dimpho Intelligence Studio" description="Release 1–4 control centre for personality, RAG knowledge, production app intelligence and privacy-filtered autonomous learning." icon={Brain} defaultOpen><DimphoIntelligenceStudio/></HubPanel><HubPanel title="Dimpho Release Control" description="Release 5–8 control plane for governed tools, customer memory, workflow persistence, evaluation gates, model routing, fine-tuning and rollback." icon={ShieldCheck} defaultOpen><DimphoReleaseControl/></HubPanel><AdminOSServiceIntelligence/><AutomationQueueContent/><HubPanel title="Advanced AdminOS Controls" description="Release controls, agents, workflows, approvals and lower-level automation administration." icon={Bot}><AdminOSMasterContent/></HubPanel></TabsContent>
