...
export default function AdminOSTwilioSetup({ integrations, onRefresh }: { integrations: AnyRow[]; onRefresh: () => Promise<void> | void }) {
      ...
      <CardHeader>
        <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
          <div>
            ...
            <p className="mt-1 max-w-3xl text-sm text-muted-foreground">Finish WhatsApp Business and Programmable Voice in three controlled steps. Credential values stay in Supabase Edge Function secrets and are never displayed back in AdminOS.</p>
          </div>
          <Badge variant="outline">Production onboarding</Badge>
        </div>
      </CardHeader>
      <CardContent className="space-y-5">
        ...
        <div className="rounded-2xl border bg-muted/20 p-4">
          <div className="grid gap-4 lg:grid-cols-3">
            <SetupStep number="1" title="Twilio account + senders" text="Use your upgraded Twilio account. Register the ResKonnect WhatsApp sender through Twilio Self Sign-up and buy/provision a South African Voice-capable number if you want AI calling." />
            <SetupStep number="2" title="Add Supabase secrets" text="Copy Account SID/Auth Token plus the approved WhatsApp sender and Voice number into Supabase Edge Function secrets. Also add the webhook URLs shown in the channel cards." />
            <SetupStep number="3" title="Test & activate" text="Return here and run Test provider. WhatsApp is marked connected after a verified Twilio account test. Voice stays standby until you explicitly switch AI Voice on in Phase 10." />
          </div>
