...
begin
  ...
  v_period := case
    ...
    when v_cycle='semester' then least(2,greatest(1,coalesce(nullif(p_patch->>'academic_period','')::smallint,1)))
    when v_cycle='trimester' then least(3,greatest(1,coalesce(nullif(p_patch->>'academic_period','')::smallint,1)))
    else null
  end;
