import { claimFollowup, FollowupBlocked } from "../_shared/studentFollowups.ts";
...
async function twilioSend(to: string, body: string | null, templateSid?: string | null, templateVars?: any) {
  ...
  if (templateSid) {
    ...
    if (templateVars && Object.keys(templateVars).length) form.set("ContentVariables", JSON.stringify(templateVars));
  } else form.set("Body", body || "");
  const r = await fetch(`https://api.twilio.com/2010-04-01/Accounts/${accountSid}/Messages.json`, {
    method: "POST",
    headers: { Authorization: basicTwilio(), "Content-Type": "application/x-www-form-urlencoded" },
    body: form,
  });
  const d = await r.json().catch(() => ({}));
...
function johannesburgHour() {
  const parts = new Intl.DateTimeFormat("en-ZA", { timeZone: "Africa/Johannesburg", hour: "2-digit", hour12: false }).formatToParts(new Date());
...
async function authorized(req: Request, service: any) {
  ...
  if (cronToken) {
    ...
    if (data?.secret_value && cronToken.length === data.secret_value.length) {
      let diff = 0;
      for (let i = 0; i < cronToken.length; i++) diff |= cronToken.charCodeAt(i) ^ data.secret_value.charCodeAt(i);
      if (diff === 0) return { ok: true, actor: "scheduler", userId: null };
    }
  }
...
async function executeStep(service: any, enrollment: any, step: any, contact: any, sequence: any) {
  ...
  if (step.action_type === "create_task") {
    const task = await service.from("conversion_automation_tasks").insert({
      task_type: step.config?.task_type || "followup",
      source_type: "adminos_followup_enrollment",
      source_id: enrollment.id,
      user_id: contact.profile_user_id || null,
      owner_scope: "admin",
      status: "pending",
      priority: step.config?.priority || "normal",
      due_at: new Date().toISOString(),
      summary: step.config?.summary || `Follow up with ${contact.full_name || "contact"}`,
      payload: { release: 3, phase: 7, sequence_key: sequence.sequence_key, enrollment_id: enrollment.id, contact_id: contact.id },
    }).select("id").single();
  ...
  if (!tmpl.data) {
    await service.from("adminos_followup_attempts").insert({ ...attemptBase, status: "failed", executed_at: new Date().toISOString(), error_message: "message_template_missing" });
    return { status: "failed", reason: "message_template_missing" };
  }

  const permission = await communicationAllowed(service, contact.id, channel, tmpl.data.purpose);
  if (!permission.allowed) {
    const retryLater = permission.reason === "quiet_hours";
    await service.from("adminos_followup_attempts").insert({ ...attemptBase, status: retryLater ? "planned" : "blocked", executed_at: retryLater ? null : new Date().toISOString(), output: { reason: permission.reason } });
    return { status: retryLater ? "planned" : "blocked", reason: permission.reason, retryLater };
  }
  ...
  if (channel === "email") {
    if (!contact.email) {
      await service.from("adminos_followup_attempts").insert({ ...attemptBase, status: "blocked", executed_at: new Date().toISOString(), output: { reason: "email_missing" } });
      return { status: "blocked", reason: "email_missing" };
    }
    const outbox = await service.from("adminos_email_outbox").insert({
      contact_id: contact.id,
      source_type: "followup_enrollment",
      source_id: enrollment.id,
      to_email: contact.email,
      subject: subject || "ResKonnect follow-up",
      body_text: body,
      risk_level: tmpl.data.risk_level || "green",
      status: "queued",
      idempotency_key: idem,
      metadata: { release: 3, phase: 7, sequence_key: sequence.sequence_key, template_key: templateKey },
    }).select("id").single();
    if (outbox.error) throw outbox.error;
    try {
      const sent = await gmailSend(contact.email, subject || "ResKonnect follow-up", body);
      ...
      await service.from("adminos_email_outbox").update({ status: error === "gmail_not_configured" ? "queued" : "failed", last_error: error }).eq("id", outbox.data.id);
      await service.from("adminos_followup_attempts").insert({ ...attemptBase, status: error === "gmail_not_configured" ? "queued" : "failed", executed_at: new Date().toISOString(), output: { outbox_id: outbox.data.id }, error_message: error });
      return { status: error === "gmail_not_configured" ? "queued" : "failed", reason: error };
    }
  }

  if (channel === "whatsapp") {
    if (!contact.phone) {
      await service.from("adminos_followup_attempts").insert({ ...attemptBase, status: "blocked", executed_at: new Date().toISOString(), output: { reason: "phone_missing" } });
      return { status: "blocked", reason: "phone_missing" };
    }
    ...
    if (!windowOpen && !tmpl.data.provider_template_sid) {
      await service.from("adminos_followup_attempts").insert({ ...attemptBase, status: "blocked", executed_at: new Date().toISOString(), output: { reason: "approved_whatsapp_template_required" } });
      return { status: "blocked", reason: "approved_whatsapp_template_required" };
    }
    const outbox = await service.from("adminos_whatsapp_outbox").insert({
      contact_id: contact.id,
      thread_id: thread.data?.id || null,
      source_type: "followup_enrollment",
      source_id: enrollment.id,
      to_address: e164(contact.phone),
      body_text: windowOpen ? body : null,
      message_kind: tmpl.data.purpose,
      template_sid: !windowOpen ? tmpl.data.provider_template_sid : null,
      template_vars: !windowOpen ? { "1": firstName(contact.full_name) } : {},
      risk_level: tmpl.data.risk_level || "green",
      status: "queued",
      idempotency_key: idem,
      metadata: { release: 3, phase: 7, sequence_key: sequence.sequence_key, template_key: templateKey },
    ...
    if (outbox.error) throw outbox.error;
    try {
      await claimFollowup(service, { phone: contact.phone, userId: contact.profile_user_id, contactId: contact.id, key: `outbox:${outbox.data.id}`, care: sequence.config?.purpose === "student_care" });
      const sent = await twilioSend(contact.phone, windowOpen ? body : null, !windowOpen ? tmpl.data.provider_template_sid : null, !windowOpen ? { "1": firstName(contact.full_name) } : {});
      const now = new Date().toISOString();
      ...
      await service.from("adminos_followup_attempts").insert({ ...attemptBase, status: "sent", executed_at: now, output: { outbox_id: outbox.data.id, twilio_message_sid: sent.sid } });
      return { status: "sent", outbox_id: outbox.data.id };
    } catch (e) {
      const error = e instanceof Error ? e.message : String(e);
  ...
  await service.from("adminos_followup_attempts").insert({ ...attemptBase, status: "failed", executed_at: new Date().toISOString(), error_message: "unsupported_channel" });
  return { status: "failed", reason: "unsupported_channel" };
}

async function advance(service: any, enrollment: any, sequence: any, result: any) {
  if (result.terminal) return;
  if (result.retryLater) {
    const retry = new Date(Date.now() + 60 * 60 * 1000).toISOString();
    await service.from("adminos_followup_enrollments").update({ next_run_at: retry }).eq("id", enrollment.id);
    return;
  }
  ...
  if (!next.data) {
    await service.from("adminos_followup_enrollments").update({ status: "completed", current_step: Number(enrollment.current_step || 0) + 1, next_run_at: null, completed_at: new Date().toISOString() }).eq("id", enrollment.id);
    return;
  }
  const nextRun = new Date(Date.now() + Number(next.data.delay_minutes || 0) * 60 * 1000).toISOString();
  await service.from("adminos_followup_enrollments").update({ current_step: Number(enrollment.current_step || 0) + 1, next_run_at: nextRun }).eq("id", enrollment.id);
}
...
serve(async (req) => {
  ...
  if (action === "health") {
    const cron = await service.from("adminos_scheduler_secrets").select("secret_key").eq("secret_key", "followup_autopilot").maybeSingle();
    return json({ ok: true, scheduler_secret: Boolean(cron.data), cron_actor: authz.actor === "scheduler", release: 3, phase: 7 });
  }

  if (action === "enroll") {
    if (!authz.userId) return json({ error: "Staff authentication required for manual enrollment" }, 403);
    const sequenceKey = String(body.sequence_key || "");
    ...
    const nextRun = new Date(Date.now() + Number(first.data.delay_minutes || 0) * 60 * 1000).toISOString();
    const insert = await service.from("adminos_followup_enrollments").insert({ sequence_id: sequence.data.id, contact_id: contactId, prospect_id: body.prospect_id || null, status: "active", current_step: 0, next_run_at: nextRun, metadata: { source: "manual", enrolled_by: authz.userId, release: 3 } }).select("*").single();
  ...
  if (["pause", "resume", "cancel"].includes(action)) {
    ...
    if (!id) return json({ error: "enrollment_id is required" }, 400);
  ...
  for (const enrollment of due.data || []) {
    try {
      ...
      if (!sequence?.enabled || !contact || !step) {
        await service.from("adminos_followup_enrollments").update({ status: "completed", next_run_at: null, completed_at: new Date().toISOString(), metadata: { ...(enrollment.metadata || {}), stopped_reason: "missing_sequence_contact_or_step" } }).eq("id", enrollment.id);
        results.push({ enrollment_id: enrollment.id, status: "completed", reason: "missing_sequence_contact_or_step" });
        continue;
      }
      if (enrollment.prospect_id && Array.isArray(sequence.config?.stop_stages) && sequence.config.stop_stages.length) {
        ...
        if (prospect.data && sequence.config.stop_stages.includes(prospect.data.stage)) {
          await service.from("adminos_followup_enrollments").update({ status: "completed", next_run_at: null, completed_at: new Date().toISOString(), metadata: { ...(enrollment.metadata || {}), stopped_reason: `prospect_stage:${prospect.data.stage}` } }).eq("id", enrollment.id);
          results.push({ enrollment_id: enrollment.id, status: "completed", reason: `prospect_stage:${prospect.data.stage}` });
          continue;
        }
      }
      const result = await executeStep(service, enrollment, step, contact, sequence);
      await advance(service, enrollment, sequence, result);
      results.push({ enrollment_id: enrollment.id, step: step.step_order, ...result });
    } catch (e) {
      const message = e instanceof Error ? e.message : String(e);
      results.push({ enrollment_id: enrollment.id, status: "failed", error: message });
