// Isolated native/browser regression: fixture auth only; no live account creation or database writes.
...
try{
 ...
 let role=null;let accessCalls=0;
 ...
 let page=await context.newPage();const errors=[];const observe=p=>p.on("pageerror",e=>errors.push(e.message));observe(page);
 await page.goto(origin);await page.waitForURL("**/auth");await page.getByLabel("Account portals").waitFor();
 console.log("PASS native signed-out auth and account portal choices");
 await page.getByLabel("Email address *").fill(user.email);
 await page.getByLabel("Password *",{exact:true}).fill("FixturePassword123");
 await page.getByRole("button",{name:"Sign In",exact:true}).click();
 await page.waitForURL("**/dashboard");
 await page.getByText("Good to see you, Test.").waitFor({timeout:20000});
 assert.equal(await page.getByRole("link",{name:"Open full dashboard"}).count(),0);
 assert.equal(await page.getByRole("navigation",{name:"App account navigation"}).count(),0,"No redundant native app header");
 assert.equal(await page.getByText("Something went wrong",{exact:true}).count(),0);
 console.log("PASS direct full dashboard without duplicated native header");
 const beforeResume=accessCalls;await page.evaluate(()=>window.dispatchEvent(new Event("online")));await page.waitForTimeout(500);
 assert.equal(accessCalls,beforeResume,"resume preserves unchanged identity");
 await page.reload();await page.getByText("Good to see you, Test.").waitFor({timeout:20000});
 await page.close();page=await context.newPage();observe(page);
 await page.goto(origin);await page.waitForURL("**/dashboard");await page.getByText("Good to see you, Test.").waitFor({timeout:20000});
 console.log("PASS persistent session through reload, close/reopen");
 for(const path of ["/profile","/my-applications","/dashboard/services","/documents","/favorites","/messages","/wil","/ai","/opportunities","/findmyres","/portals"]){
  await page.goto(origin+path);await page.waitForTimeout(600);
  assert.equal(await page.getByRole("navigation",{name:"App account navigation"}).count(),0,`${path}: duplicate native header`);
 ...
 const web=await browser.newContext({serviceWorkers:"block"});const webPage=await web.newPage();await webPage.goto(origin);await webPage.waitForTimeout(800);assert.equal(new URL(webPage.url()).pathname,"/");
 console.log("PASS website retains landing page");await web.close();
