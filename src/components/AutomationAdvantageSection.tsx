...
export default function AutomationAdvantageSection(){
  return <section className="relative overflow-hidden border-y bg-gradient-to-br from-emerald-500/[.05] via-background to-violet-500/[.05] py-12 md:py-18">
    ...
    <div className="container relative mx-auto grid gap-9 px-4 sm:px-6 lg:grid-cols-[1.05fr_.95fr] lg:items-center lg:px-8">
      <div>
        <div className="flex flex-wrap gap-2"><Badge className="rounded-full gap-1.5 px-3 py-1"><Sparkles className="h-3.5 w-3.5"/> Automation-first support</Badge><Badge variant="outline" className="rounded-full px-3 py-1">ResKonnect AI · Dimpho on WhatsApp · Human handoff</Badge></div>
        <h2 className="mt-4 max-w-3xl text-3xl font-black tracking-[-.035em] md:text-5xl">Get help in seconds, not office hours.</h2>
        <p className="mt-4 max-w-2xl text-sm leading-7 text-muted-foreground md:text-base">ResKonnect AI extends to WhatsApp through Dimpho, routing accommodation, application and WIL enquiries immediately. Dimpho can guide routine steps, read current ResKonnect data and hand the full conversation to a human when judgement is required.</p>
        <div className="mt-6 grid gap-3 sm:grid-cols-2">
          ...
          <Feature icon={Building2} title="Live accommodation data" text="Current residence images, rent, location and availability feed Dimpho's responses."/>
      ...
      <div className="mx-auto w-full max-w-[440px] rounded-[38px] border-[6px] border-foreground/90 bg-background p-2 shadow-[0_38px_80px_-38px_rgba(0,0,0,.58)]">
        <div className="overflow-hidden rounded-[28px] border bg-muted/15"><div className="flex items-center gap-3 border-b bg-background/90 px-4 py-3 backdrop-blur-xl"><img src="/icon-512.png" alt="ResKonnect" className="h-10 w-10 rounded-full bg-white object-cover ring-1 ring-black/5"/><div className="min-w-0 flex-1"><p className="text-sm font-black">ResKonnect</p><p className="text-[10px] text-emerald-600">ResKonnect AI · Dimpho on WhatsApp</p></div><span className="h-2.5 w-2.5 rounded-full bg-emerald-500"/></div><div className="space-y-3 bg-gradient-to-b from-emerald-500/[.04] to-background p-4">
          ...
          <Bubble><div className="flex items-center gap-2"><Bot className="h-4 w-4 text-violet-500"/><strong>Dimpho</strong></div><p className="mt-2">Perfect. I’ll narrow the live ResKonnect listings and show the best matching options with current images and details.</p></Bubble>
