-- ResKonnect Course Match — Phase 5A / 5A.1 UP 2027 regression suite
-- Target: external Supabase project mefjzkhobkltlbmhusdh

-- Structural boundary after distinct UPOnline programme 09110003 reconciliation.
with i as (
  select id from public.institutions where slug='university-of-pretoria'
), p as (
  select * from public.programmes where institution_id=(select id from i)
)
select
  (select count(*) from p where is_active) active_programmes,
  (select count(*) from p where is_active and metadata->>'qualification_system'='nsc_ieb') domestic_programmes,
  (select count(*) from p where is_active and coalesce(metadata->>'student_facing','false')='true' and coalesce(metadata->>'matric_direct_eligible','false')='true' and metadata->>'qualification_system'='nsc_ieb') matcher_candidates,
  (select count(*) from p where is_active and metadata->>'entry_route'='faculty_placement') faculty_placement,
  (select count(*) from p where is_active and metadata->>'entry_route'='faculty_placement' and coalesce(metadata->>'student_facing','false')='true') placement_leakage,
  (select count(*) from p where is_active and metadata->>'qualification_system'<>'nsc_ieb') international_leakage,
  (select count(*) from (select slug,count(*) c from p group by slug having count(*)>1)d) duplicate_slugs,
  (select count(*) from (select metadata->>'qualification_code' code,count(*) c from p where is_active and metadata->>'qualification_code' is not null group by 1 having count(*)>1)d) duplicate_codes,
  (select count(*) from p left join public.programme_requirements r on r.programme_id=p.id and r.is_active where p.is_active and r.id is null) missing_requirements,
  (select count(*) from public.programme_requirements r join p on p.id=r.programme_id where r.is_active and r.verified_status='pending_review') pending_requirements,
  (select count(*) from public.programme_requirements r join p on p.id=r.programme_id where r.is_active and r.aps_required is null and p.metadata->>'qualification_code'<>'01130014') unexpected_null_aps,
  (select count(*) from public.programme_subject_requirements s join p on p.id=s.programme_id) subject_rows,
  (select count(*) from public.programme_subject_requirements s join p on p.id=s.programme_id where s.requirement_kind='conditional') conditional_rows,
  (select count(*) from public.programme_selection_rules s join p on p.id=s.programme_id) selection_rows,
  (select count(distinct s.programme_id) from public.programme_selection_rules s join p on p.id=s.programme_id) selection_programmes,
  (select count(*) from public.course_match_up(42,'[{"name":"English Home Language","mark":100},{"name":"Mathematics","mark":100},{"name":"Mathematical Literacy","mark":100},{"name":"Physical Sciences","mark":100},{"name":"Life Sciences","mark":100},{"name":"Accounting","mark":100},{"name":"Music","mark":100}]'::jsonb,true)) full_output_rows;

-- Expected structural values:
-- active_programmes=129, domestic_programmes=129, matcher_candidates=128,
...
-- pending_requirements=0, unexpected_null_aps=0, subject_rows=312,
-- conditional_rows=4, selection_rows=24, selection_programmes=24,
-- full_output_rows=128.

-- Behavioural parity: original 27 Phase 5A assertions + T28 UPOnline.
...
('T28',20,'[{"name":"English Home Language","mark":50}]'::jsonb,'09110003','eligible')
), r as (
