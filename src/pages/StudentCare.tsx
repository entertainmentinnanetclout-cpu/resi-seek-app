...
/** The value 3 means both semesters, not a third semester. */
const periodLabel = (value: unknown) => Number(value) === 3 ? 'Both semesters / full academic year' : Number(value) === 2 ? 'Second semester' : 'First semester';
const blankFeedback = {residence_name:'',campus:'',institution:'',stay_semester:'3',safety_rating:'',maintenance_rating:'',management_rating:'',overall_rating:'',concerns:'',improvements:'',urgent_safety_concern:false,petition_support:false,share_with_institution:false,share_identity:false};
const blankRoom = {academic_year:'2027',semester:'3',full_name:'',phone:'',campus:'',preferred_residence:'',budget:'',funding_type:'undecided',move_in_date:'',notes:'',room_type:'single'};
export default function StudentCare() {
  ...
  const load = async () => {
    try {
      ...
      setCampaigns(c.data || []); setCampaign(v => v || c.data?.[0]?.id || '');
      if(!user) {setResponses([]);setRequests([]);setError('');return;}
      const [f,r,p] = await Promise.all([
        db.from('residence_stay_feedback').select('*').eq('user_id',user.id).order('updated_at',{ascending:false}),
        db.from('single_room_waitlist').select('*').eq('user_id',user.id).order('created_at',{ascending:false}),
        db.from('profiles').select('full_name,phone,campus').eq('id',user.id).maybeSingle()
      ]);
      if(f.error || r.error || p.error) throw f.error || r.error || p.error;
      setResponses(f.data || []);setRequests(r.data || []);setError('');
      if(p.data) {
        setRoom(v => ({...v,full_name:v.full_name || p.data.full_name || '',phone:v.phone || p.data.phone || '',campus:v.campus || p.data.campus || ''}));
        setFeedback(v => ({...v,campus:v.campus || p.data.campus || ''}));
      }
  ...
  const save = async (event:React.FormEvent) => {
    ...
    if(!user) return navigate('/auth?returnTo='+encodeURIComponent(location.pathname+location.search));
    setBusy(true);
    try {
      const r = waiting ? await db.rpc('rk_join_single_room_waitlist',{p_request:room}) : await db.rpc('rk_submit_residence_feedback',{p_campaign_id:campaign,p_feedback:feedback});
      if(r.error) throw r.error;
      toast.success(waiting ? 'Single-room request saved' : 'Residence feedback saved');
      await load();
    } catch(e:any) {toast.error(e.message || 'Unable to save your request');}
    finally {setBusy(false);}
  };
  const existing = responses.find(r => r.campaign_id === campaign);
  return <DashboardLayout><SEO title={waiting ? 'Single-Room Waiting List | ResKonnect' : 'Residence Feedback & Student Care | ResKonnect'} description="Request a single room for your chosen term or the full year, share residence feedback and track student care with ResKonnect." />
    <main className="mx-auto max-w-6xl space-y-6 px-4 py-8 sm:px-6">
      <section className="rounded-3xl bg-[#071326] p-7 text-white sm:p-10"><Badge className="bg-[#F5B32F] text-[#071326]">STUDENTS FIRST</Badge><h1 className="mt-4 text-3xl font-black sm:text-4xl">{waiting ? 'Strictly single rooms.' : 'Your experience matters.'}</h1><p className="mt-4 max-w-3xl text-sm leading-7 text-white/75">{waiting ? 'Find a single room for first semester, second semester or both. A semester selection tells us when you need accommodation; it does not mean you have to leave after one semester. Joining the list does not reserve a bed.' : 'Tell us about your experience in first semester, second semester or across the full academic year. Students can stay in the same residence for both semesters, subject to their actual accommodation agreement. Your feedback must reflect your real stay, not a presumed semester-only placement.'}</p></section>
      ...
      <div className="grid items-start gap-6 lg:grid-cols-[1.3fr_1fr]">
        <Panel title={waiting ? 'Find a single room for me' : 'Residence stay feedback'}><form onSubmit={save} className="grid gap-4 sm:grid-cols-2">
          {waiting ? <>
            ...
            <Field label="When do you need the room?"><select className={selectStyle} value={room.semester} onChange={e => setRoom({...room,semester:e.target.value})}><option value="3">Both semesters / full academic year</option><option value="1">First semester</option><option value="2">Second semester</option></select></Field>
            <p className="rounded-xl bg-primary/5 p-3 text-xs leading-5 text-muted-foreground sm:col-span-2">Your search period is not your move-out date. You may request accommodation for both semesters; the actual length of stay is determined by the residence offer and lease.</p>
            ...
            {existing && <div className="rounded-xl bg-muted/40 p-3 text-sm sm:col-span-2">You have already responded. <button type="button" className="font-bold underline" onClick={() => setFeedback({...blankFeedback,...existing,stay_semester:String(existing.stay_semester)})}>Load my feedback to update it</button></div>}
            ...
            <Field label="Which part of your stay does this feedback cover?"><select className={selectStyle} value={feedback.stay_semester} onChange={e => setFeedback({...feedback,stay_semester:e.target.value})}><option value="3">Both semesters / full academic year</option><option value="1">First semester only</option><option value="2">Second semester only</option></select></Field>
            <p className="rounded-xl bg-primary/5 p-3 text-xs leading-5 text-muted-foreground sm:col-span-2">This records the period you are reviewing. Selecting a semester does not end or change your accommodation agreement.</p>
        ...
        <div className="space-y-5"><Panel title={waiting ? 'My single-room requests' : 'My feedback & responses'}>
          ...
          {(waiting ? requests : responses).map(r => <article key={r.id} className="space-y-3 rounded-xl border p-4"><div className="flex flex-wrap justify-between gap-2"><p className="font-bold">{waiting ? r.academic_year+' · '+periodLabel(r.semester) : r.residence_name+' · '+periodLabel(r.stay_semester)}</p><Badge variant="secondary">{friendlyStatus(r.status)}</Badge></div><p className="text-sm text-muted-foreground">{r.campus}</p>{r.staff_response && <p className="whitespace-pre-wrap text-sm"><strong>ResKonnect: </strong>{r.staff_response}</p>}{r.institution_response && <p className="whitespace-pre-wrap text-sm"><strong>Institution: </strong>{r.institution_response}</p>}{waiting && r.status !== 'withdrawn' && <Button size="sm" variant="outline" disabled={busy} onClick={async () => {setBusy(true);try {const {error} = await db.rpc('rk_withdraw_single_room',{p_id:r.id});if(error) throw error;toast.success('Request withdrawn');await load();}catch(e:any){toast.error(e.message || 'Could not withdraw request');}finally{setBusy(false);}}}>Withdraw request</Button>}</article>)}
        </Panel><Panel title="Quality throughout the year"><p className="text-sm leading-6 text-muted-foreground">ResKonnect can collect feedback for either semester or a full-year stay. We record concerns, seek responses and use year-end feedback to guide improvements. Proactive WhatsApp follow-ups are limited to one per month, at least 30 days apart. You can message us whenever you need help.</p></Panel></div>
      </div>
