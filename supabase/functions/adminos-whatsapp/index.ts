import { claimFollowup, isFollowupOutbox, FollowupBlocked } from "../_shared/studentFollowups.ts";
...
async function ensureThread(service: any, address: string, contactId: string | null) {
  ...
  const created = await service.from("adminos_whatsapp_threads").insert({
    contact_id: contactId, channel_address: wa(address), normalized_address: norm, status: "open", metadata: { source: "adminos_whatsapp" },
  }).select("*").single();
...
async function sendItem(service: any, item: any) {
  ...
  if (item.template_sid) {
    ...
    form.set("Body", String(item.body_text || ""));
  }
  ...
  try {
    if (isFollowupOutbox(item, windowOpen)) await claimFollowup(service, { phone: item.to_address, contactId: item.contact_id, key: `outbox:${item.id}`, care: item.metadata?.purpose === "student_care" });
    ...
    await service.from("adminos_whatsapp_messages").upsert({
      thread_id: thread.id, contact_id: item.contact_id, twilio_message_sid: sent.sid, direction: "outbound",
      from_address: wa(fromNumber), to_address: wa(item.to_address), body_text: item.body_text || null,
      message_kind: item.message_kind || "transactional", status: sent.status === "queued" ? "queued" : "sent",
      risk_level: item.risk_level, confidence: item.confidence, agent_run_id: item.agent_run_id,
      sent_at: now, metadata: { source: "adminos_outbox", outbox_id: item.id, template_sid: item.template_sid || null },
    ...
    await service.from("adminos_whatsapp_outbox").update({ status: e instanceof FollowupBlocked ? "blocked" : "failed", last_error: message }).eq("id", item.id);
    if (e instanceof FollowupBlocked) return { id: item.id, status: "blocked", reason: message };
    throw e;
  }
}
...
serve(async (req) => {
  ...
  if (action === "queue") {
    ...
    if (risk !== "green") {
      const approval = await service.from("adminos_approval_requests").insert({
        request_type: "whatsapp_reply", title: `Approve ${risk}-risk WhatsApp message`, summary: String(body.body_text || "WhatsApp template message").slice(0, 400),
        entity_type: "whatsapp_thread", entity_id: thread.id, requested_action: { channel: "whatsapp", to: wa(toAddress), message_kind: kind }, risk_level: risk,
        requested_by_type: "agent",
      }).select("id").single();
      if (approval.error) throw approval.error;
      approvalId = approval.data.id; status = "awaiting_approval";
    }
    const inserted = await service.from("adminos_whatsapp_outbox").insert({
      contact_id: contactId, thread_id: thread.id, source_type: body.source_type || "manual", source_id: body.source_id || null,
      to_address: e164(toAddress), body_text: body.body_text || null, message_kind: kind, template_sid: body.template_sid || null,
      template_vars: body.template_vars || {}, risk_level: risk, confidence: body.confidence ?? null, status, approval_id: approvalId,
      agent_run_id: body.agent_run_id || null, idempotency_key: idempotency, send_after: body.send_after || new Date().toISOString(), metadata: body.metadata || {},
    }).select("*").single();
  ...
  if (action === "flush") {
    ...
    for (const item of due.data || []) {
      try {
        const result = await sendItem(service, item); results.push(result);
        if (result.status === "sent") sent++; else if (result.status === "blocked") blocked++;
