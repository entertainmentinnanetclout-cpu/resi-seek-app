import AssistanceWorkspaceTools from "@/components/applications/AssistanceWorkspaceTools";
...
const CreatorAssistanceIntake = () => {
  ...
  const [form, setForm] = useState<any>({ applicant_name:"", email:"", phone:"", student_number:"", identity_number:"", applicant_stage:"matriculant", campus:"Not yet selected", course:"", year_of_study:"", institution_type:"university", target_institutions:"", qualification_interests:"", funding_type:"undecided", intake_year:"2027", intake_semester:"1", student_notes:"" });

  useEffect(() => {
    const load = async () => {
      if (!slug) return;
      const { data: creatorData } = await (supabase as any).from("creator_partners").select("id,slug,display_name,handle,platform,status,profile_image_url,assistance_fee").eq("slug", slug).eq("status", "active").maybeSingle();
      ...
      const [{ data: profile }, { data: existing }] = await Promise.all([
        ...
        (supabase as any).from("creator_assistance_cases").select("*").eq("creator_id", creatorData.id).eq("student_user_id", user.id).eq("intake_year", Number(form.intake_year)).maybeSingle(),
      ]);
      setCaseRow(existing || null);
  ...
  const createCase = async () => {
    ...
    const payload = {
      ...
      funding_type: form.funding_type, intake_year: Number(form.intake_year || 2027), intake_semester: Number(form.intake_semester), service_fee: Number(creator.assistance_fee || 0), status: "documents_pending", consent_status: "granted", student_notes: form.student_notes.trim() || null,
    };
  ...
  const markReady = async () => {
    ...
    setCaseRow(data); toast.success("Your application pack is ready for partner assistance.");
  };
  ...
  return <DashboardLayout>
    <SEO noIndex title={`Application Assistance with ${creator.display_name} | ResKonnect`} description="Secure partner-assisted application preparation through ResKonnect." />
    <div className="mx-auto max-w-5xl space-y-6 px-4 py-8 sm:px-6">
      <section className="overflow-hidden rounded-3xl bg-[#071326] p-6 text-white md:p-9"><div className="flex flex-col gap-5 sm:flex-row sm:items-center"><div className="flex h-16 w-16 shrink-0 items-center justify-center overflow-hidden rounded-2xl border border-white/20 bg-white/10">{creator.profile_image_url ? <img src={creator.profile_image_url} alt="" className="h-full w-full object-cover"/> : <img src={BRAND.logos.icon} alt={BRAND.name} className="h-12 w-12 rounded-xl bg-white p-1"/>}</div><div><Badge className="bg-[#F5B32F] text-[#071326] hover:bg-[#F5B32F]">RESKONNECT APPLICATION ASSISTANCE</Badge><h1 className="mt-3 text-3xl font-black">{creator.display_name} × ResKonnect</h1><p className="mt-2 text-sm text-white/70">Prepare your information and documents in one secure workspace. Your partner can assist with the application process after you explicitly grant access.</p></div></div></section>

      <div className="rounded-xl border bg-muted/30 p-4 text-sm">Partner service fee: <strong>R{Number(creator.assistance_fee || 0).toFixed(2)}</strong> per annual assistance case, covering all institution choices. ResKonnect maximum: R100. Existing cases keep their agreed fee.</div>
      {!caseRow ? <Card><CardHeader><CardTitle>Your application assistance brief</CardTitle></CardHeader><CardContent className="grid gap-4 md:grid-cols-2">
        <Field label="Intake year"><Input type="number" min="2026" max="2100" value={form.intake_year} onChange={e=>setForm({...form,intake_year:e.target.value})}/></Field>
        <Field label="Semester"><Select value={form.intake_semester} onValueChange={v=>setForm({...form,intake_semester:v})}><SelectTrigger><SelectValue/></SelectTrigger><SelectContent><SelectItem value="1">First semester</SelectItem><SelectItem value="2">Second semester</SelectItem></SelectContent></Select></Field>
        <Field label="Institution type"><Select value={form.institution_type} onValueChange={v=>setForm({...form,institution_type:v})}><SelectTrigger><SelectValue/></SelectTrigger><SelectContent><SelectItem value="university">University</SelectItem><SelectItem value="tvet">TVET</SelectItem><SelectItem value="private">Private college</SelectItem><SelectItem value="other">Other</SelectItem></SelectContent></Select></Field>
        ...
        <Field label="Qualifications / courses you want" className="md:col-span-2"><Textarea value={form.qualification_interests} onChange={(e)=>setForm({...form,qualification_interests:e.target.value})} placeholder="Tell your partner what you want to study."/></Field>
        <Field label="Anything the partner should know" className="md:col-span-2"><Textarea value={form.student_notes} onChange={(e)=>setForm({...form,student_notes:e.target.value})}/></Field>
        <div className="md:col-span-2 rounded-2xl border bg-muted/30 p-4"><div className="flex items-start gap-3"><Checkbox id="creator-consent" checked={consent} onCheckedChange={(value)=>setConsent(value===true)}/><Label htmlFor="creator-consent" className="text-sm leading-6">I authorize <strong>{creator.display_name}</strong> to view the application information and documents I upload to this assistance case so they can help me prepare and submit applications. I can revoke this access later. I agree to the total service fee of <strong>R{Number(creator.assistance_fee || 0).toFixed(2)}</strong> for this annual case. No additional partner service fee may be added.</Label></div><div className="mt-3 flex items-start gap-2 text-xs text-muted-foreground"><ShieldCheck className="mt-0.5 h-4 w-4 shrink-0 text-primary"/>This does not give the partner access to your entire ResKonnect account, accommodation portal data, passwords or unrelated private files.</div></div>
        ...
        <Card><CardContent className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between"><div><div className="flex items-center gap-2"><CheckCircle2 className="h-5 w-5 text-emerald-600"/><p className="font-black">Assistance case active</p><Badge variant="outline">{String(caseRow.status).replaceAll("_"," ")}</Badge></div><p className="mt-1 text-sm text-muted-foreground">Target: {(caseRow.target_institutions||[]).join(", ") || "Not specified"}</p></div>{caseRow.consent_status === "granted" && <Button variant="outline" className="text-destructive" onClick={()=>void revoke()}>Revoke partner access</Button>}</CardContent></Card>
        {caseRow.consent_status === "granted" ? <Card><CardHeader><CardTitle className="flex items-center gap-2"><FileCheck2 className="h-5 w-5 text-primary"/>Application documents</CardTitle></CardHeader><CardContent><ApplicationDocumentUploader caseId={caseRow.id} applicantStage={caseRow.applicant_stage} onReadinessChange={setDocsReady}/>{caseRow.status === "documents_pending" && <Button className="mt-5 w-full" disabled={!docsReady} onClick={()=>void markReady()}>I have uploaded the required documents</Button>}</CardContent></Card> : <Card><CardContent className="p-8 text-center"><ShieldCheck className="mx-auto h-8 w-8 text-muted-foreground"/><p className="mt-3 font-bold">Partner access revoked</p><p className="mt-1 text-sm text-muted-foreground">This case is closed and the partner can no longer read its application documents.</p></CardContent></Card>}
        <AssistanceWorkspaceTools row={caseRow}/>
      </>}
    </div>
  </DashboardLayout>;
};
