...
serve(async(req)=>{
  ...
  try{
    const r=await fetch(`${supabaseUrl}/functions/v1/reskonnect-brain`,{method:"POST",headers:{Authorization:`Bearer ${serviceKey}`,apikey:serviceKey,"Content-Type":"application/json"},body:JSON.stringify({action:"chat",agent_key:"voice_service",channel:"voice",thread_ref:callId,message:speech,contact_id:call.contact_id,context:{channel:"voice",purpose:call.purpose,automated_outbound:true,do_not_make_financial_or_legal_commitments:true}})});const d=await r.json().catch(()=>({}));
