...
function ThreeDOverlay({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // Primary renderer: Google's native Photorealistic 3D Map3DElement.
      try {
      ...
      // Reliability fallback: Google vector map with genuine WebGL tilt/heading
      // and 3D building geometry where the vector basemap exposes it.
      try {
        ...
        console.error("ResMap vector 3D fallback failed", vectorError);
        throw vectorError;
      }
    })().catch((error) => {
      if (disposed) return;
      ...
      <header className="pointer-events-none absolute inset-x-0 top-0 z-20 flex items-start justify-between gap-3 px-3 pt-[calc(10px+env(safe-area-inset-top))] sm:px-5">
        <div className="pointer-events-auto rounded-[22px] border border-white/20 bg-slate-950/78 px-4 py-3 shadow-2xl backdrop-blur-xl">
          ...
          <p className="mt-1 max-w-[68vw] text-[11px] text-white/70">
            {engine === "vector"
              ? "WebGL vector 3D fallback is active. Drag, rotate, tilt and zoom; 3D building geometry appears where Google exposes it."
      ...
      {status === "failed" && (
        <div className="absolute inset-0 z-10 grid place-items-center p-6">
          <div className="max-w-md rounded-[28px] border border-white/15 bg-slate-950/90 p-6 text-center shadow-2xl backdrop-blur-xl">
            ...
            <p className="mt-2 text-sm text-white/70">ResMap attempted both Photorealistic 3D and the WebGL vector 3D fallback. The standard live 2D map remains available.</p>
