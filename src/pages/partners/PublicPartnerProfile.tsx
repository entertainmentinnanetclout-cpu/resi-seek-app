...
export default function PublicPartnerProfile(){
 ...
 useEffect(()=>{let live=true;setLoading(true);setProfile(null);setScopes([]);setLink('');void(async()=>{
  ...
  const residenceNames:string[]=[];
  if(data.kind==='residence'&&residenceIds.length){const names=await(supabase as any).from('residences').select('name').in('id',residenceIds).eq('is_visible',true).limit(40);if(!live)return;for(const r of names.data||[])residenceNames.push(r.name);}
  setProfile(data);setScopes([...allowed.filter(s=>s.scope_kind!=='residence').map(s=>s.scope_value),...residenceNames]);
