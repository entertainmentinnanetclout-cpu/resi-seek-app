...
import { BarChart3, Building2, Home, LogOut, Menu, LayoutDashboard, RefreshCw, TrendingUp, ShoppingCart, Film, Cpu, GraduationCap, Users, ClipboardCheck, Handshake } from "lucide-react";
    ...
    <Sidebar collapsible="icon">
      <SidebarHeader className="border-b p-4">
        <div className="flex items-center gap-2">
          ...
          {!collapsed && <div className="min-w-0"><img src={BRAND.logos.full} alt={BRAND.name} className="h-6 w-auto object-contain" /><Badge variant="outline" className="text-[10px] mt-1">{roleLabels[staffRole || "admin"]}</Badge></div>}
        </div>
      </SidebarHeader>
      <SidebarContent><SidebarGroup><SidebarGroupContent><SidebarMenu>
        {navItems.map((item) => { const isActive = location.pathname === item.path; return <SidebarMenuItem key={item.path}><SidebarMenuButton asChild isActive={isActive} tooltip={item.label}><Link to={item.path}><item.icon className="w-4 h-4" /><span>{item.label}</span></Link></SidebarMenuButton></SidebarMenuItem>; })}
      </SidebarMenu></SidebarGroupContent></SidebarGroup></SidebarContent>
      <SidebarFooter className="border-t p-2 space-y-1"><SidebarMenu>
...
const AdminLayout = ({ children }: AdminLayoutProps) => {
  ...
  if (!isGodMode && staffRole) {
    ...
    if (staffRole === 'tvet_lead') navigate('/tvet-dashboard'); else navigate('/dashboard');
    return null;
  }
  return <SidebarProvider><div className="min-h-screen flex w-full"><AdminSidebar staffRole={staffRole} /><div className="flex-1 flex flex-col min-w-0"><header className="h-12 flex items-center justify-between border-b bg-card px-4 shrink-0 sticky top-0 z-40"><SidebarTrigger /><ThemeToggle /></header><main className="flex-1 p-4 md:p-6 lg:p-8">{children}</main></div></div></SidebarProvider>;
};
