...
begin
  ...
  if v_path is not null then
    insert into public.seo_index_queue(path,action) values(v_path,v_action);
  end if;
...
begin
  ...
  v_public := coalesce(new.is_published,false) or coalesce(old.is_published,false);
  v_action := case when tg_op='INSERT' then 'created' when tg_op='DELETE' then 'deleted' else 'updated' end;
...
begin
  v_slug := coalesce(new.slug, old.slug);
  v_visible := coalesce(new.is_visible,false) or coalesce(old.is_visible,false);
  if v_slug is not null and v_visible then
    insert into public.seo_index_queue(path,action)
    values('/find-my-res/'||v_slug, case when tg_op='INSERT' then 'created' when tg_op='DELETE' then 'deleted' else 'updated' end);
