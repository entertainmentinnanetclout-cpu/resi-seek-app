...
import ResMapNativeSafe from "@/components/resmap/ResMapNativeSafe";
...
// Never evaluate the crash-prone Google Photorealistic 3D bundle in Android's WebView.
const ResMapWeb3D = lazy(() => import("@/components/resmap/ResMapExperiencePremiumV4"));

interface SmartSearchBarProps { filters: ResidenceFilters; updateFilter: <K extends keyof ResidenceFilters>(key: K, value: ResidenceFilters[K]) => void; resultCount: number; totalCount: number; }
...
export function SmartSearchBar({ filters, updateFilter, resultCount, totalCount }: SmartSearchBarProps) {
  ...
  const openMap = () => {
    setDiscoveryOpen(false); setMapOpen(true);
    const url = new URL(window.location.href); url.searchParams.set("view", "map"); url.searchParams.delete("discovery");
    if (native) url.searchParams.delete("mode");
    window.history.replaceState({}, "", url);
  };
  const closeMap = () => {
    setMapOpen(false);
    const url = new URL(window.location.href); url.searchParams.delete("view"); url.searchParams.delete("mode");
    window.history.replaceState({}, "", url);
  };
  const openDiscovery = () => {
    setMapOpen(false); setDiscoveryOpen(true);
    const url = new URL(window.location.href); url.searchParams.delete("view"); url.searchParams.delete("mode"); url.searchParams.set("discovery", "discover");
    window.history.replaceState({}, "", url);
  };
  const closeDiscovery = () => {
    setDiscoveryOpen(false);
    const url = new URL(window.location.href); url.searchParams.delete("discovery"); url.searchParams.delete("group");
    window.history.replaceState({}, "", url);
  };
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-12 lg:px-8">
        <div className="mb-6 text-center">
          <div className="mb-3 flex flex-wrap justify-center gap-2">
            ...
            <button onClick={openMap} className="inline-flex items-center gap-2 rounded-full border border-blue-500/20 bg-blue-500/10 px-3 py-1.5 text-xs font-black text-blue-700 transition hover:bg-blue-500/15 dark:text-blue-300"><MapPinned className="h-3.5 w-3.5" />{native ? "Nearby map" : "Map & 3D tools"}</button>
          </div>
        ...
        <div className="space-y-4 rounded-xl border bg-card/80 p-4 shadow-lg backdrop-blur-lg sm:p-6">
          ...
          <div className="flex flex-col gap-3 border-t border-border/50 pt-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="grid grid-cols-2 gap-2 sm:flex"><Button onClick={openDiscovery} variant="outline" className="h-10 rounded-full px-4"><Sparkles className="mr-2 h-4 w-4 text-violet-500" />Smart Match</Button><Button onClick={openMap} className="h-10 rounded-full bg-gradient-to-r from-blue-600 to-cyan-500 px-5 text-white shadow-lg"><MapPinned className="mr-2 h-4 w-4" />{native ? "Nearby Map" : "Map & 3D"}</Button></div>
          </div>
        </div>
      </div>
    </div>
    {mapOpen && (native ? <ResMapNativeSafe filters={filters} updateFilter={updateFilter} resetFilters={resetMapFilters} onClose={closeMap} /> : <Suspense fallback={<div className="fixed inset-0 z-[230] grid place-items-center bg-background">Loading ResMap…</div>}><ResMapWeb3D filters={filters} updateFilter={updateFilter} resetFilters={resetMapFilters} onClose={closeMap} /></Suspense>)}
