...
const ResidenceLayout = () => {
  ...
  const NavContent = () => <div className="flex h-full min-h-0 flex-col overflow-hidden">
    <div className="shrink-0 border-b p-5"><Link to="/residence" onClick={() => setIsOpen(false)} className="flex items-center gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-primary/10 text-primary"><Building2 className="h-5 w-5" /></div><div className="min-w-0"><p className="font-black tracking-tight">ResKonnect</p><p className="text-xs text-muted-foreground">Landlord Portal · Property OS</p></div></Link>{residence && <p className="mt-4 truncate rounded-lg bg-muted/70 px-3 py-2 text-sm font-semibold">{residence.name}</p>}</div>
    <nav className="min-h-0 flex-1 space-y-1 overflow-y-auto overscroll-contain p-3">{navItems.map((item) => { const active = item.match(location.pathname); return <Link key={item.path} to={item.path} onClick={() => setIsOpen(false)} className={cn("flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-semibold transition-colors", active ? "bg-primary text-primary-foreground shadow-sm" : "text-muted-foreground hover:bg-muted hover:text-foreground")}><item.icon className="h-5 w-5 shrink-0" /><span>{item.label}</span>{item.label === "Applications" && pendingCount > 0 && <Badge variant={active ? "secondary" : "destructive"} className="ml-auto">{pendingCount}</Badge>}</Link>; })}</nav>
    <div className="shrink-0 space-y-1 border-t bg-card p-3">{residence && <Button asChild variant="ghost" className="w-full justify-start"><Link to={`/res/${residence.id}`} target="_blank"><ExternalLink className="mr-3 h-4 w-4" />View public listing</Link></Button>}<Button variant="ghost" className="w-full justify-start text-destructive hover:text-destructive" onClick={handleLogout}><LogOut className="mr-3 h-4 w-4" />Sign out</Button></div>
  </div>;

  return <div className="flex min-h-screen min-w-0 overflow-x-hidden bg-muted/20">
    <aside className="fixed inset-y-0 left-0 hidden h-dvh w-64 overflow-hidden border-r bg-card lg:block"><NavContent /></aside>
    <header className="fixed inset-x-0 top-0 z-50 flex h-16 min-w-0 items-center justify-between border-b bg-background/95 px-3 backdrop-blur sm:px-4 lg:hidden"><div className="min-w-0"><p className="font-bold">Landlord Portal</p><p className="max-w-[150px] truncate text-xs text-muted-foreground sm:max-w-[220px]">{residence?.name || "Loading residence..."}</p></div><div className="flex shrink-0 items-center gap-1 sm:gap-2"><Button variant="ghost" size="icon" className="relative" onClick={() => navigate("/residence/inbox?status=new")} aria-label="Applications needing attention"><Bell className="h-5 w-5" />{pendingCount > 0 && <span className="absolute -right-0.5 -top-0.5 flex h-4 min-w-4 items-center justify-center rounded-full bg-destructive px-1 text-[10px] font-bold text-destructive-foreground">{pendingCount > 9 ? "9+" : pendingCount}</span>}</Button><ThemeToggle /><Sheet open={isOpen} onOpenChange={setIsOpen}><SheetTrigger asChild><Button variant="outline" size="icon"><Menu className="h-5 w-5" /></Button></SheetTrigger><SheetContent side="left" className="h-dvh w-[86vw] max-w-72 overflow-hidden p-0"><NavContent /></SheetContent></Sheet></div></header>
    <main className="min-w-0 flex-1 overflow-x-hidden lg:ml-64"><div className="sticky top-0 z-30 hidden h-16 items-center justify-between border-b bg-background/90 px-6 backdrop-blur lg:flex"><div><p className="text-sm font-semibold">{residence?.name || "Loading residence..."}</p><p className="text-xs text-muted-foreground">Listing quality, pricing, recruitment, applications and conversion analytics.</p></div><div className="flex items-center gap-2"><Button variant="outline" className="relative" onClick={() => navigate("/residence/inbox?status=new")}><Inbox className="mr-2 h-4 w-4" />Applications{pendingCount > 0 && <Badge variant="destructive" className="ml-2">{pendingCount}</Badge>}</Button><ThemeToggle /></div></div><div className="min-w-0 p-3 pt-20 sm:p-4 sm:pt-20 md:p-6 md:pt-20 lg:p-8"><Outlet context={{ residence }} /></div></main>
  </div>;
};
