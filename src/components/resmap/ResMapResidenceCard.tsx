...
import { BedDouble, Bike, Building2, Bus, Car, CheckCircle2, Clock3, Footprints, MapPin, Route, ShieldCheck, Sparkles, Wifi, X } from "lucide-react";
    ...
    <section className="pointer-events-auto w-full overflow-hidden rounded-[28px] border border-white/60 bg-white/95 shadow-2xl backdrop-blur-xl dark:border-white/10 dark:bg-slate-950/95 sm:max-w-[430px]">
      <div className="relative h-40 bg-gradient-to-br from-[#061a33] via-[#0b3d70] to-[#0e91a8]">
        ...
        <div className="absolute bottom-3 left-3 right-3 flex items-end justify-between gap-3 text-white">
          <div className="min-w-0"><p className="truncate text-lg font-black">{residence.name}</p><p className="flex items-center gap-1 truncate text-xs text-white/85"><MapPin className="h-3.5 w-3.5" />{residence.place_label || residence.address || residence.campus}</p></div>
      ...
      <div className="space-y-4 p-4">
        ...
        <div className="grid grid-cols-2 gap-2">
          <Button variant="outline" onClick={onRoute}><Route className="mr-2 h-4 w-4" />Route here</Button>
          <Button onClick={() => navigate(`/find-my-res/${slug}?intent=secure`)}><CheckCircle2 className="mr-2 h-4 w-4" />View & secure</Button>
        </div>
        <Button className="w-full" variant="secondary" onClick={() => navigate(`/find-my-res/${slug}`)}><ShieldCheck className="mr-2 h-4 w-4" />Full residence details</Button>
        {routeInfo?.provider === "geodesic-fallback" && <p className="text-[10px] leading-relaxed text-muted-foreground"><Clock3 className="mr-1 inline h-3 w-3" />Road routing was temporarily unavailable, so this journey is an estimate. The residence location remains mapped from stored coordinates.</p>}
      </div>
    </section>
  );
}
