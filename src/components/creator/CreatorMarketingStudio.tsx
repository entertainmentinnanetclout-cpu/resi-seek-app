...
const CreatorMarketingStudio = ({ creatorName, referralCode, creatorSlug }: { creatorName: string; referralCode: string; creatorSlug: string }) => {
  ...
  const referralUrl = useMemo(() => `www.reskonnect.org/creator/${creatorSlug || "partner"}`, [creatorSlug]);
  ...
  return <Card className="overflow-hidden border-primary/20"><CardHeader><CardTitle className="flex items-center gap-2"><Sparkles className="h-5 w-5 text-primary" />Co-branded Marketing Studio</CardTitle></CardHeader><CardContent className="space-y-5">
    ...
    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><p className="text-xs leading-5 text-muted-foreground"><ImageIcon className="mr-1 inline h-3.5 w-3.5" />Generated locally in your browser using the creator landing route that preserves campaign attribution.</p><Button onClick={download}><Download className="mr-2 h-4 w-4" />Download PNG</Button></div>
  </CardContent></Card>;
};
