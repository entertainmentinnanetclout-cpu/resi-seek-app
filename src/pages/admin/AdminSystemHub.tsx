...
const legacy:Record<string,string>={
  adminos:"automation",automation:"automation",intelligence:"automation",dimpho:"automation",
...
const AdminSystemHub=()=>{
  ...
  return <AdminLayout>
    ...
    <div className="adminos-command-centre min-w-0 max-w-full space-y-6 overflow-x-hidden">
      <section className="max-w-full overflow-hidden rounded-[30px] border bg-gradient-to-br from-background to-muted/35 p-5 shadow-sm sm:p-6"><div className="flex min-w-0 flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><div className="min-w-0"><div className="flex max-w-full flex-wrap gap-2"><Badge className="rounded-full">Simplified AdminOS</Badge><Badge variant="outline" className="rounded-full">4 command hubs</Badge><Badge variant="outline" className="rounded-full">Dimpho Intelligence R1–R8</Badge></div><h1 className="mt-3 break-words text-3xl font-black tracking-tight">ResKonnect Command Centre</h1><p className="mt-2 max-w-3xl break-words text-sm leading-6 text-muted-foreground">Communications, automation, operations and platform control now share one AdminOS. Dimpho Intelligence Studio and R5–R8 release controls live inside Automation so personality, ResKonnect knowledge, production app intelligence, governed tools, customer memory, evaluation and model operations remain controlled instead of becoming another maze of admin pages.</p></div><div className="grid min-w-0 grid-cols-2 gap-2 text-center sm:grid-cols-4"><MiniStat value="4" label="Hubs"/><MiniStat value="1" label="WhatsApp inbox"/><MiniStat value="24/7" label="Automation"/><MiniStat value="Live" label="Supabase"/></div></div></section>
