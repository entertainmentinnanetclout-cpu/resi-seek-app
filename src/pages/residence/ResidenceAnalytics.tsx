...
const ResidenceAnalytics = () => {
  ...
  const load = useCallback(async () => {
    ...
    const [apps, resv, crm, demandRes, partner] = await Promise.all([
      ...
      supabase.functions.invoke("housing-intelligence", { body: { action: "property_partner", residence_id: residence.id, days: 90 } }),
    ]);
    ...
    if (!partner.error) setPartnerIntel((partner.data as any)?.partner || null);
    setLoading(false);
  }, [residence?.id]);
  ...
  const analytics = useMemo(() => {
    const approved = applications.filter((row) => ["approved", "conditionally_approved"].includes(row.status)).length;
    const decided = applications.filter((row) => ["approved", "conditionally_approved", "rejected"].includes(row.status)).length;
    const res2027 = reservations.filter((row) => Number(row.academic_year) === 2027 && row.status !== "cancelled");
    const placed = leads.filter((row) => row.stage === "placed").length;
    const activeLeads = leads.filter((row) => row.stage !== "lost").length;
    const contacted = leads.filter((row) => !["new", "lost"].includes(row.stage)).length;
    const nsfas = [...applications, ...res2027].filter((row) => row.funding_type === "nsfas").length;
    const privateCount = [...applications, ...res2027].filter((row) => row.funding_type === "private").length;
    const statuses = new Map<string, number>();
    leads.forEach((row) => statuses.set(row.stage, (statuses.get(row.stage) || 0) + 1));
    return {
      apps: applications.length,
      approved,
      approvalRate: decided ? Math.round((approved / decided) * 100) : 0,
      reservations2027: res2027.length,
      placed,
      activeLeads,
      contacted,
      ...
      nsfas,
      privateCount,
      stages: [...statuses.entries()].sort((a, b) => b[1] - a[1]),
    };
  ...
  const metric = (label: string, value: string | number, note: string, Icon: typeof Users) => (
    ...
    <>
      ...
      <div className="mx-auto max-w-7xl space-y-6">
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          ...
          <div className="flex gap-2"><Button variant="outline" onClick={() => void load()}><RefreshCw className={`mr-2 h-4 w-4 ${loading ? "animate-spin" : ""}`} />Refresh</Button><Button onClick={() => navigate("/residence/crm")}><Target className="mr-2 h-4 w-4" />Open CRM</Button></div>
        </div>
        ...
        {partnerIntel && (
          <Card className="border-primary/25 bg-primary/[0.025]">
            <CardHeader><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><CardTitle className="flex items-center gap-2"><BarChart3 className="h-5 w-5 text-primary" />Property Partner Benchmark</CardTitle><CardDescription>Phase 22 · Your private 90-day performance compared with the campus market.</CardDescription></div><Badge variant="outline">{benchmark.campus_residences || 0} campus residences</Badge></div></CardHeader>
            <CardContent>
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
                <div className="rounded-2xl border bg-background p-4"><p className="text-xs text-muted-foreground">Application index</p><p className="mt-1 text-2xl font-black">{market.application_index ?? "—"}</p><p className="text-xs text-muted-foreground">100 = campus average</p></div>
                <div className="rounded-2xl border bg-background p-4"><p className="text-xs text-muted-foreground">Reservation index</p><p className="mt-1 text-2xl font-black">{market.reservation_index ?? "—"}</p><p className="text-xs text-muted-foreground">100 = campus average</p></div>
                <div className="rounded-2xl border bg-background p-4"><p className="text-xs text-muted-foreground">Listing views</p><p className="mt-1 text-2xl font-black">{performance.views || 0}</p><p className="text-xs text-muted-foreground">ResMap activity</p></div>
                <div className="rounded-2xl border bg-background p-4"><p className="text-xs text-muted-foreground">Listing saves</p><p className="mt-1 text-2xl font-black">{performance.saves || 0}</p><p className="text-xs text-muted-foreground">High-intent signal</p></div>
                <div className="rounded-2xl border bg-background p-4"><p className="text-xs text-muted-foreground">Price vs campus</p><p className="mt-1 text-2xl font-black">{market.price_delta == null ? "—" : `${Number(market.price_delta) >= 0 ? "+" : ""}R${Number(market.price_delta).toLocaleString("en-ZA")}`}</p><p className="text-xs text-muted-foreground">Campus avg R{Number(benchmark.average_private_price || 0).toLocaleString("en-ZA")}</p></div>
              </div>
              <p className="mt-4 text-xs leading-5 text-muted-foreground">Benchmark data is aggregate. ResKonnect does not expose another residence’s private CRM, student records, or identifiable applicant data.</p>
            </CardContent>
          </Card>
        )}
