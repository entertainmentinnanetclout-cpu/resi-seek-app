...
alter table public.adminos_demand_snapshots
  add constraint adminos_demand_snapshots_academic_year_check
  check (academic_year between 2020 and 2100);
...
create or replace function public.luna_academic_supply_live(
...
create or replace function public.luna_academic_demand_heat(
  p_academic_year integer default extract(year from current_date)::integer,
...
with direct as (
  ...
  select public.housing_intel_campus_key(w.campus) campus_key,
         count(*)::bigint whatsapp_signals,
         count(*) filter(where lower(coalesce(w.funding,''))='nsfas')::bigint nsfas_wa,
         count(*) filter(where lower(coalesce(w.funding,'')) in ('private','self-funded','self_funded'))::bigint private_wa
  from public.adminos_whatsapp_conversion_leads w
  where w.academic_year=p_academic_year
    and w.created_at>=now()-make_interval(days=>greatest(1,least(coalesce(p_days,90),730)))
  group by 1
), scoped_web as (
  select public.housing_intel_campus_key(e.campus) campus_key,
         count(*)::bigint search_signals
  from public.adminos_demand_events e
  where e.metadata->>'academic_year'=p_academic_year::text
    and e.event_type in ('residence_search','filter_change')
    and e.created_at>=now()-make_interval(days=>greatest(1,least(coalesce(p_days,90),730)))
  group by 1
), scoped_feed as (
  select public.housing_intel_campus_key(r.campus) campus_key,
  ...
  select campus_key from direct
  union select campus_key from app
  union select campus_key from resv
  union select campus_key from wa
  union select campus_key from scoped_web
  union select campus_key from scoped_feed
), merged as (
  select k.campus_key,
         coalesce(d.direct_count,0)+coalesce(a.application_signals,0)+coalesce(rv.reservation_signals,0)+coalesce(w.whatsapp_signals,0) demand_count,
         coalesce(d.active_searchers,0) active_searchers,
         coalesce(d.nsfas_demand,0)+coalesce(a.nsfas_apps,0)+coalesce(rv.nsfas_resv,0)+coalesce(w.nsfas_wa,0) nsfas_demand,
         coalesce(d.private_demand,0)+coalesce(a.private_apps,0)+coalesce(rv.private_resv,0)+coalesce(w.private_wa,0) private_demand,
         d.average_budget,
         coalesce(sw.search_signals,0)+coalesce(sf.search_signals,0) scoped_search_signals
  from keys k
  left join direct d using(campus_key)
  left join app a using(campus_key)
  left join resv rv using(campus_key)
  left join wa w using(campus_key)
  left join scoped_web sw using(campus_key)
  left join scoped_feed sf using(campus_key)
)
select m.campus_key,
       coalesce(c.name,m.campus_key) campus_name,
       m.demand_count,
       m.active_searchers,
       m.nsfas_demand,
       m.private_demand,
       round(m.average_budget,0) average_budget,
       m.scoped_search_signals,
       least(100,round(12*ln(1+m.demand_count::numeric)+4*ln(1+m.scoped_search_signals::numeric)))::int demand_index
from merged m
left join public.resmap_campuses c on c.campus_key=m.campus_key and c.is_active=true
order by demand_index desc,demand_count desc;
$function$;

revoke all on function public.luna_academic_demand_heat(integer,integer) from public,anon;
grant execute on function public.luna_academic_demand_heat(integer,integer) to authenticated,service_role;
