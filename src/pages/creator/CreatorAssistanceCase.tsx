...
const CreatorAssistanceCase = () => {
  ...
  const load = async () => {
    if (!user || !caseId) { setLoading(false); return; }
    setLoading(true);
  ...
  if (!row) return <DashboardLayout><div className="mx-auto max-w-xl px-4 py-24 text-center"><LockKeyhole className="mx-auto h-10 w-10 text-muted-foreground"/><h1 className="mt-4 text-2xl font-black">Case unavailable</h1><p className="mt-2 text-sm text-muted-foreground">You do not have access to this application case, or the student has revoked consent.</p><Button className="mt-5" onClick={()=>navigate("/application-assistance")}>Back to Application assistance</Button></div></DashboardLayout>;

  const canManage = user?.id !== row.student_user_id;
  return <DashboardLayout>
    <SEO noIndex title={`Application Assistance | ${row.applicant_name} | ResKonnect`} description="Protected partner application assistance workspace." />
    <div className="mx-auto max-w-6xl space-y-6 px-4 py-8 sm:px-6">
      <div><Button asChild variant="ghost" className="-ml-3"><Link to="/application-assistance"><ArrowLeft className="mr-2 h-4 w-4"/>Application assistance</Link></Button></div>
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr,.9fr]">
        <div className="space-y-6">
          <Card><CardHeader><CardTitle>Applicant information</CardTitle></CardHeader><CardContent className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="sm:col-span-2 rounded-xl border bg-muted/30 p-3 text-xs leading-5 text-muted-foreground"><ShieldCheck className="mr-1 inline h-4 w-4 text-primary"/>These details are visible because the applicant explicitly consented to partner-assisted applications. Do not copy, export or use them outside the student's application work.</div>
          </CardContent></Card>

          <Card><CardHeader><CardTitle className="flex items-center gap-2"><FileText className="h-5 w-5 text-primary"/>Application documents</CardTitle></CardHeader><CardContent>{row.consent_status === "granted" && row.status !== "closed" && <div className="mb-5"><ApplicationDocumentUploader caseId={row.id} ownerUserId={row.student_user_id} applicantStage={row.applicant_stage}/></div>}{docs.length === 0 ? <div className="rounded-xl border border-dashed p-8 text-center text-sm text-muted-foreground">No documents uploaded yet.</div> : <div className="space-y-2">{docs.map((doc)=><div key={doc.id} className="flex items-center gap-3 rounded-xl border p-3"><div className="rounded-lg bg-primary/10 p-2 text-primary"><FileText className="h-4 w-4"/></div><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold">{doc.file_name}</p><p className="text-xs text-muted-foreground">{doc.document_type.replaceAll("_"," ")}</p></div><Button size="sm" variant="outline" onClick={()=>void openDoc(doc)}><ExternalLink className="mr-2 h-4 w-4"/>Open</Button></div>)}</div>}</CardContent></Card>
        </div>

        <div className="space-y-6">
          {canManage && <Card><CardHeader><CardTitle>Application workflow</CardTitle></CardHeader><CardContent className="space-y-4">
            ...
            <div className="space-y-1.5"><Label>Partner working notes</Label><Textarea value={notes} onChange={(e)=>setNotes(e.target.value)} className="min-h-[180px]" placeholder="Requirements checked, institution portal progress, next follow-up..."/></div>
            <Button className="w-full" onClick={()=>void save()} disabled={saving}><Save className="mr-2 h-4 w-4"/>{saving ? "Saving..." : "Save progress"}</Button>
          </CardContent></Card>}
          {row.status === "submitted" || row.status === "completed" ? <Card className="border-emerald-500/30 bg-emerald-500/[0.04]"><CardContent className="p-5"><CheckCircle2 className="h-6 w-6 text-emerald-600"/><p className="mt-3 font-black">Submission tracked</p><p className="mt-1 text-sm text-muted-foreground">This outcome is now part of the partner's measurable ResKonnect conversion record.</p></CardContent></Card> : null}
        </div>
      </div>
      <AssistanceWorkspaceTools row={row} canManage={canManage}/>
    </div>
  </DashboardLayout>;
};
