...
export default function ImmersiveResidence() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const [m, d, f, r, published] = await Promise.all([
        ...
        db.from("virtual_tour_publications").select("public_token,tour_id,version_number,published_at").eq("residence_id", id).order("published_at", { ascending: false }).limit(1).maybeSingle(),
      ]);
      if (!active) return;
      setMedia(m.data || []); setTwin(d.data || null); setFloors(f.data || []); setRooms(r.data || []); setV2Tour(published.data || null); setLoading(false);
    })();
    return () => { active = false; };
  }, [slug]);
  ...
  return <main className="min-h-screen bg-background pb-24">
    <SEO title={`${residence.name} Immersive 3D Tour | ResKonnect`} description={`Explore ${residence.name} in ResKonnect Immersive Living: 3D residence view, verified 360 tours, room inventory and interactive reservation.`} />
    <header className="sticky top-0 z-40 border-b bg-background/92 backdrop-blur-xl"><div className="mx-auto flex max-w-7xl items-center justify-between gap-3 px-4 py-3"><Button variant="ghost" size="sm" onClick={() => navigate(-1)}><ArrowLeft className="mr-2 h-4 w-4" />Back</Button><div className="min-w-0 text-center"><p className="truncate text-sm font-black">{residence.name}</p><p className="text-[10px] uppercase tracking-[.18em] text-muted-foreground">ResKonnect Immersive Living</p></div><Button size="sm" onClick={() => navigate(`/find-my-res/${residence.slug || residence.id}?intent=secure`)}>Secure</Button></div></header>
    ...
    <div className="mx-auto max-w-7xl space-y-6 px-4 py-6">
      <section className="overflow-hidden rounded-[30px] border bg-card shadow-sm"><div className="grid gap-0 lg:grid-cols-[1.35fr_.65fr]">
        ...
        <aside className="space-y-5 border-t p-5 lg:border-l lg:border-t-0">
          <div><div className="flex flex-wrap gap-2">{residence.accepts_nsfas && <Badge className="bg-emerald-600">NSFAS</Badge>}{residence.is_tut_accredited && <Badge className="bg-blue-600">TUT verified</Badge>}{residence.has_wifi && <Badge variant="outline"><Wifi className="mr-1 h-3 w-3" />Wi-Fi</Badge>}{residence.reservations_2027_open && <Badge variant="outline"><CalendarDays className="mr-1 h-3 w-3" />2027 open</Badge>}{v2Tour?.public_token && <Badge className="bg-[#071326] text-white"><Crown className="mr-1 h-3 w-3 text-[#F5B32F]" />Gold 360 V2</Badge>}</div><h1 className="mt-3 text-3xl font-black tracking-tight">{residence.name}</h1><p className="mt-2 flex items-start gap-2 text-sm text-muted-foreground"><MapPin className="mt-0.5 h-4 w-4 shrink-0" />{residence.canonical_address || residence.address || residence.campus}</p>{!exactLocation && <p className="mt-2 rounded-xl border border-amber-300 bg-amber-50 p-2.5 text-xs text-amber-900">Approximate area pin. Use the Google Maps check below for the exact entrance while ResKonnect verifies this residence.</p>}</div>
          ...
          {v2Tour?.public_token && <Button asChild className="w-full bg-[#F5B32F] font-black text-[#071326] hover:bg-[#ffd16e]"><Link to={`/tour/${v2Tour.public_token}`}><View className="mr-2 h-4 w-4" />Enter 4K Gold 360 Tour</Link></Button>}
