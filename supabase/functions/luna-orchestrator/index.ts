...
const canonicalize=(value:unknown)=>{
  ...
  const fixed=text
    .replaceAll("https://reskonnect.co.za","https://www.reskonnect.org")
    .replaceAll("https://www.reskonnect.co.za","https://www.reskonnect.org")
    .replaceAll("http://reskonnect.co.za","https://www.reskonnect.org")
    .replaceAll("http://www.reskonnect.co.za","https://www.reskonnect.org");
  try{return JSON.parse(fixed);}catch{return value;}
};
...
async function runContentCycle(service:any,authz:any,body:any){
  ...
  try{
    ...
    if(existingPlan&&!body?.force){
      if(runId)await service.from("adminos_agent_runs").update({status:"succeeded",output:{skipped:true,reason:"unchanged_fingerprint",content_plan_id:existingPlan.id},completed_at:new Date().toISOString()}).eq("id",runId);
    ...
    const fallback={
      ...
      hook:`${opportunity.campus_name}: start your accommodation search with ResKonnect.`,
      offer:"Explore accommodation listings and current application information on ResKonnect.",
      cta:"Open Find My Res and explore current options.",
    ...
    const rawDraft=generated.draft&&typeof generated.draft==="object"?generated.draft:fallback;
    const draft=canonicalize(rawDraft) as any;
    ...
    if(runId)await service.from("adminos_agent_runs").update({status:"succeeded",output:{campaign_id:campaignId,content_plan_id:contentPlanId,campaign_code:campaignCode,quality_score:quality,risk_level:risk,manual_publish_required:true,networks},completed_at:new Date().toISOString()}).eq("id",runId);
...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    if(runId)await service.from("adminos_agent_runs").update({status:"succeeded",output:{snapshot_id:snapshot.id,ranked:ranked.slice(0,8),source_counts:sourceCounts},completed_at:new Date().toISOString()}).eq("id",runId);
