...
import { ArrowLeft, BadgeCheck, ChevronLeft, ChevronRight, Expand, MapPin, Play, Pause, Rotate3D } from "lucide-react";
...
export default function VirtualTourViewerPage() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (snapshot?.tour?.id) void recordTourEvent({ tourId: snapshot.tour.id, eventType: "tour_open", viewerSession: viewerSession.current, anonymousId: localStorage.getItem("rk360-anon") || undefined });
  ...
  useEffect(() => {
    ...
    void recordTourEvent({ tourId: data.tour.id, sceneId, eventType: "scene_view", viewerSession: viewerSession.current });
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => setSceneId((current) => {
      ...
      if (index >= scenes.length - 1) { setGuided(false); if (data?.tour?.id) void recordTourEvent({ tourId: data.tour.id, eventType: "guided_complete", viewerSession: viewerSession.current }); return scenes[index]?.id || current; }
  ...
  return <main className="min-h-[100dvh] bg-[#050b14] text-white">
    ...
    <div className="relative h-[100dvh] overflow-hidden">
      <VirtualTourPanorama panoramaUrl={scene.panorama_url} hotspots={hotspots} connections={connections} onNavigate={setSceneId} />
      ...
      <header className="pointer-events-none absolute inset-x-0 top-0 z-20 flex items-start justify-between gap-3 p-3 pt-[max(.75rem,env(safe-area-inset-top))] sm:p-5">
        ...
        <div className="pointer-events-auto max-w-[70vw] rounded-2xl border border-white/15 bg-black/55 px-4 py-3 text-center backdrop-blur-xl"><div className="flex items-center justify-center gap-2"><BadgeCheck className="h-4 w-4 text-emerald-400"/><p className="truncate text-sm font-black">{data.residence?.name || data.tour.title}</p></div><p className="mt-1 truncate text-[10px] uppercase tracking-[.18em] text-[#F5B32F]">ResKonnect verified 360 · {data.tour.quality_tier}</p></div>
        <button onClick={() => { document.documentElement.requestFullscreen?.(); void recordTourEvent({ tourId: data.tour.id, sceneId: scene.id, eventType: "fullscreen", viewerSession: viewerSession.current }); }} className="pointer-events-auto grid h-11 w-11 place-items-center rounded-full border border-white/20 bg-black/55 backdrop-blur" aria-label="Fullscreen"><Expand className="h-5 w-5"/></button>
      </header>
