    ...
    <header className="sticky top-0 z-50 border-b border-border bg-background/85 backdrop-blur-md">
      <div className="container mx-auto flex max-w-7xl items-center gap-3 px-4 py-2.5 sm:px-6 lg:px-8">
        ...
        <nav className="hidden items-center gap-1 md:flex">
          {PUBLIC_NAV.map((item) => (
            <Link key={item.to} to={item.to} className={`rounded-md px-3 py-2 text-sm font-medium transition-colors ${isActive(item.to) ? "text-primary" : "text-foreground/80 hover:text-primary"}`}>
              {item.label}
            </Link>
          ))}
        </nav>
              ...
              <div className="mt-10 flex flex-col gap-2">
                {PUBLIC_NAV.map((item) => (
                  <SheetClose key={item.to} asChild>
                    <Link to={item.to} className="rounded-lg px-3 py-3 text-lg font-medium transition-colors hover:bg-muted">{item.label}</Link>
                  </SheetClose>
                ))}
