...
export interface SpecialistNavItem {
  value: string;
  label: string;
  ...
  return (
    <Sidebar collapsible="icon">
        ...
        <div className="flex items-center gap-2">
          ...
          {!collapsed && (
            <div className="min-w-0">
              ...
              <Badge variant="outline" className="text-[10px] mt-0.5">{roleLabel}</Badge>
            </div>
          )}
        </div>
      </SidebarHeader>
      ...
      <SidebarContent>
        <SidebarGroup>
          <SidebarGroupContent>
            <SidebarMenu>
              {navItems.map((item) => {
                const isActive = location.pathname === basePath && activeTab === item.value;
                return (
                  <SidebarMenuItem key={item.value}>
                    <SidebarMenuButton
                      ...
                      tooltip={item.label}
                      onClick={() => {
                        if (location.pathname !== basePath) navigate(`${basePath}?tab=${item.value}`);
                        else setSearchParams({ tab: item.value });
                      }}
                    >
      ...
      <SidebarFooter className="border-t p-2 space-y-1">
        <SidebarMenu>
          <SidebarMenuItem>
            <SidebarMenuButton onClick={() => { toast.info("Refreshing..."); window.location.reload(); }} tooltip="Refresh">
