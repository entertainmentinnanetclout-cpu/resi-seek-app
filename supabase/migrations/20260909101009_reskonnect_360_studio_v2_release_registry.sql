create table if not exists public.virtual_tour_release_registry (
  release_key text primary key, release_name text not null, status text not null default 'active', phases integer[] not null default '{}', metadata jsonb not null default '{}'::jsonb, updated_at timestamptz not null default now()
);
...
returns boolean language sql stable security definer set search_path=public as $$
  select public.virtual_tour_is_admin() or exists(
    ...
    where a.residence_id=p_residence_id and a.is_active=true
      and (a.user_id=auth.uid() or (a.user_id is null and lower(a.email)=lower(coalesce(auth.jwt()->>'email',''))))
  );
$$;
