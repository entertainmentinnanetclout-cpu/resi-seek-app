...
try {
  ...
  await expectFullDashboard();
  console.log("PASS native full dashboard immediately after sign-in without duplicate app strip");
  ...
  await page.waitForURL("**/dashboard");
  await expectFullDashboard();
  console.log("PASS full dashboard after reload, close/reopen and unchanged-identity resume");
  for (const path of ["/profile", "/my-applications", "/dashboard/services", "/documents", "/favorites", "/messages", "/wil", "/ai", "/opportunities", "/findmyres", "/portals"]) {
    ...
    console.log("PASS route without native app strip " + path);
  }
  ...
  console.log("PASS sidebar sign-out removes persisted session");
