...
const STATIC_PAGE_PATHS = [
  "/", "/get-started", "/find", "/findmyres", "/living", "/living/student-accommodation", "/living/private-rentals", "/living/parents", "/housing-intelligence",
  ...
  "/partners", "/partners/landlords", "/partners/institutions", "/bursaries", "/discounts", "/events", "/campus-news", "/roommates", "/affiliates",
  "/student-accommodation", "/student-accommodation/pretoria", "/student-accommodation/pretoria-west", "/student-accommodation/near-tut",
  "/student-accommodation/near-tut-pretoria-west", "/student-accommodation/tut", "/student-accommodation/soshanguve",
  "/student-accommodation/near-tshwane-south-tvet", "/student-accommodation/tvet", "/student-accommodation/university", "/student-accommodation/nsfas-accredited",
  ...
  "/guides/how-to-find-safe-student-accommodation", "/guides/student-accommodation-pretoria-west", "/guides/tvet-application-checklist",
  "/guides/university-application-checklist", "/guides/what-documents-do-you-need-for-student-accommodation", "/terms", "/privacy",
];

const escapeXml = (value = "") => String(value).replaceAll("&", "&amp;").replaceAll("<", "&lt;").replaceAll(">", "&gt;").replaceAll('"', "&quot;").replaceAll("'", "&apos;");
const normalizePath = (path = "/") => !path || path === "/" ? "/" : `/${String(path).replace(/^\/+|\/+$/g, "")}`;
const absoluteUrl = (path = "/") => { const normalized = normalizePath(path); return `${SITE_URL}${normalized === "/" ? "" : normalized}`; };
...
function dedupeRows(rows) {
  const byLoc = new Map();
  for (const row of rows) { if (!row?.loc) continue; const existing = byLoc.get(row.loc); if (!existing || (!existing.lastmod && row.lastmod)) byLoc.set(row.loc, row); }
  return [...byLoc.values()].sort((a, b) => a.loc.localeCompare(b.loc));
}
function urlset(rows) {
  const urls = dedupeRows(rows).map(({ loc, lastmod }) => `<url><loc>${escapeXml(loc)}</loc>${lastmod ? `<lastmod>${escapeXml(new Date(lastmod).toISOString())}</lastmod>` : ""}</url>`).join("");
  return `<?xml version="1.0" encoding="UTF-8"?><urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">${urls}</urlset>`;
}
function sitemapIndex() {
  const entries = ["pages", "residences", "properties", "opportunities"].map((name) => `<sitemap><loc>${SITE_URL}/sitemaps/${name}.xml</loc></sitemap>`).join("");
...
async function rowsFor(type) {
  if (type === "pages") {
    ...
    return dedupeRows([...staticRows, ...rows.map((row) => ({ loc: absoluteUrl(row.path), lastmod: row.updated_at }))]);
  }
  if (type === "residences") {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!["pages", "residences", "properties", "opportunities"].includes(type)) return res.status(404).send("<?xml version=\"1.0\" encoding=\"UTF-8\"?><urlset xmlns=\"http://www.sitemaps.org/schemas/sitemap/0.9\"></urlset>");
    return res.status(200).send(urlset(await rowsFor(type)));
  } catch (error) {
